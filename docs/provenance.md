...
Keep the receipt, original `.xcresult` and unchanged source tree for later analysis. DerivedData may be removed after successful capture: analysis uses the frozen export, still verifies the original result bundle digest, and does not search for replacement build products. Older receipts without frozen exports use the legacy `xccov` adapter when they retain a matching captured Xcode selection; its column-less aggregates can remain ambiguous for same-line closures. Receipts lacking a captured Xcode selection must be recaptured for Xcode-scoped analysis.
